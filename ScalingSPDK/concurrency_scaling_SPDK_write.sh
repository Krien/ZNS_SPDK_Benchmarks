...
# Create FIO test
#define job
jobname="./tmp/concurrency_write_autogenerated_spdk_$job_cnt.fio"
jobglob=$(cat <<'EOF'
...
LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$SPDK_DIR/build/lib" LD_PRELOAD="$SPDK_DIR/build/fio/spdk_nvme" $FIO_DIR/fio "$jobname" --output="./data/concurrency_scaling_write_spdk_${job_cnt}.json" --output-format="json"
done
